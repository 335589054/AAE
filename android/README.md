...
| 新增歌曲导入资源包 | 新增时可选择一个 zip：自动按规则解压到 `assets/songs/<id>/`，并自动用包内的 **`songdata.json`** / `songlist` / `slst` 填充曲名、曲师、BPM 等字段**与难度列表**（id 与曲包以表单填写为准） |
...
> **导入资源包时请只放单曲资源**：压缩包里应包含该歌曲的谱面（`.aff`）、音频（`.ogg`）、封面（`.jpg`/`.png`），
> 以及可选的 `songdata.json`（用于自动填充曲名、曲师、BPM、**难度列表**等）。支持「根目录扁平」/
> 「`<歌曲目录>/…`」/「`assets/songs/<id>/…`」三种结构；非资源文件（如 `readme.txt`、曲包横幅）会被跳过。
> **把整个 APK 改名成 `.zip` 导入会被明确拒绝并提示正确做法**——否则会把 `AndroidManifest.xml`、
> `classes.dex`、各歌曲的文件等无关内容混进歌曲目录。
